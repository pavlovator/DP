...
class CNN_CV1_250(nn.Module):
    def __init__(self, direction):
        super(CNN_CV1_250, self).__init__()
        ...
        self.fc1 = nn.Linear(32 * 5 * 11, 120)
...
for direction in range(0, 360, 45):
    ...
    train_set = ScaledDatasetC("uniform_train/*", height=250, direction=direction)
    ...
    save_history(history, "history_models/250px/CNN_CV1_dir_{:}_250px.json".format(direction))
    save_model(num_epochs, batch_size, net, optimizer, criterion, "trained_models/250px/CNN_CV1_dir_{:}_250px.pt".format(direction))

    test_set = ScaledDatasetC("uniform_test/*", height=250, direction=direction)
    test(net, test_set, device, "test_models/250px/CNN_CV1_dir_{:}_250px.txt".format(direction))
