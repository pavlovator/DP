import torch
from utils.dataloaders2 import VNET2Dataset
...
train_set = VNET2Dataset(xmls_folder=xml_folder_train, height=img_height, img_folder=img_folder, config=out_conf, gauss_folder = g_folder)
val_set = VNET2Dataset(xmls_folder=xml_folder_val, height=img_height, img_folder=img_folder, config=out_conf, gauss_folder = g_folder)
...
net = VNET(input_shape=img_shape, latent_dim=128, output_dim=output_size, in_channels=4)
net.to(device)
optimizer = optim.Adam(net.parameters(), lr=learning_rate)

train(net, optimizer, criterion, val_set, device, num_epochs, train_loader, verbose_interval, history)
plot_history(history, "VNET2")


xml_folder = "../../data/test_set/*"
test_set = VNET2Dataset(xmls_folder=xml_folder, height=img_height, img_folder=img_folder, config=out_conf, gauss_folder = g_folder)
